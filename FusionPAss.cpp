...
//Controlla se il blocco successore del branch di guardia del primo loop è l'header del secondo loop.
bool Guardia(Loop *First, Loop *Second) {
  BasicBlock *B = First->getLoopGuardBranch()->getSuccessor(0);
  // Noi non vogliamo il pre-header, ci interessa il successore non loop
  if (B == First->getLoopPreheader()) {
    B = First->getLoopGuardBranch()->getSuccessor(1);//prima c'era 0
  }
...
//Verifica se i loop sono adiacenti. Se il blocco di uscita del primo loop è il preheader del secondo loop, 
//ritorna vero. Inoltre, gestisce i casi in cui i loop sono protetti da una guardia.
...
//Verifica se i loop hanno lo stesso numero di iterazioni utilizzando Scalar Evolution Analysis (SCEV).
...
//Verifica la dominanza e la post-dominanza dei loop utilizzando Dominator Tree (DT) e PostDominator Tree (PDT).
bool Controllo_3(Loop *First, Loop *Second, DominatorTree &DT, PostDominatorTree &PDT) {
...
//Dovrebbe verificare che i loop siano indipendenti utilizzando le informazioni sulle dipendenze.
